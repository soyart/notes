			...
			- Before we go in and see how `schedinit` works, let's first look at how OS memory gets managed with `mSpan`,`stackpool`, and `stackLarge`
				- ```
				  ...
				  └───────────────────────────┬────────────────────────────┘
				                              ...
				                              ▼
				  ┌────────────────────────────────────────────────────────┐
				  │  3. RUNTIME POOL LAYER (stackpool)                     │
			...
			- ### Recap: Go memory management
				...
				- {{renderer code_diagram,mermaid}}
					- ```mermaid
					  flowchart TD
					      %% Define Styles
					      classDef os style fill:#f9f9f9,stroke:#333,stroke-width:2px;
					      classDef global style fill:#e1f5fe,stroke:#0288d1,stroke-width:2px;
					      classDef local style fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px;
					      classDef execution style fill:#fff3e0,stroke:#ef6c00,stroke-width:2px;
					  
					      %% Layer 1: Operating System
					      subgraph OS_Layer [1. Operating System Layer]
					          OS["Virtual Memory Space <br> (Allocated via mmap / VirtualAlloc)"]
					      end
					      class OS_Layer os;
					  
					      %% Layer 2: Central Global Allocator
					      subgraph Global_Layer [2. Central Global Layer - REQUIRES LOCKS]
					          mSpan["mSpans <br> (Wraps OS virtual pages & slices them into fixed-size slots)"]
					          
					          subgraph Central_Pools [Central Pools]
					              stackpool["stackpool <br> (Array of mSpans for Small Stacks &lt; 32KB)"]
					              mcentral["mcentral <br> (Central Heap Allocator Pools)"]
					          end
					      end
					      class Global_Layer global;
					  
					      %% Layer 3: Per-Processor Local Cache
					      subgraph Local_Layer [3. Per-P Local Cache Layer - LOCK-FREE]
					          subgraph P_Cache [Processor P Cache]
					              mcache["mcache <br> (Thread-Local Cache attached to P)"]
					              p_stackcache["p.stackcache <br> (Private stash of small stack segments)"]
					          end
					      end
					      class Local_Layer local;
					      ...
					      %% Flow Connections
					      OS -->|1. Hands raw address chunks| mSpan
					      mSpan -->|2. Categorized into size-classes| Central_Pools
					      
					      %% Fast Path (Lock-Free)
					      p_stackcache -->|3a. FAST PATH: Instant Lock-Free assignment| G
					      mcache -->|3b. FAST PATH: Instant Heap allocation| G
					      
					      %% Slow Path (Locked Refills)
					      stackpool -.->|4a. SLOW PATH: Refills local stash when empty| p_stackcache
					      mcentral -.->|4b. SLOW PATH: Refills heap cache when empty| mcache
					  
					      %% Formatting link styles
					      linkStyle 2,3 stroke:#2e7d32,stroke-width:2.5px,stroke-dasharray: 0;
					      linkStyle 4,5 stroke:#c62828,stroke-width:2px,stroke-dasharray: 5 5;
